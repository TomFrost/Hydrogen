...
class MSSQLFormatter extends QueryFormatter {
	...
	protected function parseEXEC(&$clause) {
		return $str = 'EXEC ';
	}

	protected function parseSTOREDPROCEDURE(&$clause) {
		$str = '';
		if(!isset($clause['storedProcedure']))
			throw new InvalidSQLException('Stored Procedure argument has no stored procedure');
		$str .= $clause['storedProcedure'];
		if(isset($clause['arguments']) && $clause['arguments'])
			$str .= ' ' . $clause['arguments'];
		return $str;
	}
	...
	protected function parseLIMIT(&$clause) {
		...
		if (isset($clause['offset']))
			throw new InvalidSQLException('Row Offset is not supported in MSSQL');
		$str .= '(' . $clause['rowcount'] . ')';
		return $str;
	}
