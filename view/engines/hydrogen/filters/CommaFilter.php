...
class CommaFilter implements Filter {

	public static function applyTo($string, $args, &$escape, $phpfile) {
		if (count($args) !== 1)
			throw new TemplateSyntaxException("The 'comma' filter requires exactly one argument.");
		$escape = false;
		return 'number_format('. $string . ',' . $args[0]->getValue($phpfile) . ",'.',',')";
	}
}
